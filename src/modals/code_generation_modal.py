...
class CodeGenerationModal(discord.ui.Modal, title="CodeGenerationModal"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if self.league.value.upper() not in leagues:
            raise Exception(
                f"{self.league.value} not found! Please check for typos and try again."
            )
        ...
        if self.team1.value.lower() not in [team.lower() for team in teams]:
            raise Exception(
                f"{self.team1.value} not found! Please check for typos and try again."
            )
        if self.team2.value.lower() not in [team.lower() for team in teams]:
            raise Exception(
                f"{self.team2.value} not found! Please check for typos and try again."
            )
        ...
        metadata: Metadata = Metadata(
            series_id=series_id,
            league=self.league.value,
            teams=[self.team1.value, self.team2.value],
            game=game,
        )
